...
<#
.SYNOPSIS
    Deploy Deep Research App to Azure App Service as Docker container
...
.PARAMETER WebAppName
    Azure Web App name (optional, defaults to app-deep-research-{rg})
...
try {
    Write-Info "========================================="
    Write-Info "Deep Research App - Azure Deployment"
    Write-Info "========================================="
    Write-Info ""
    ...
    if ([string]::IsNullOrWhiteSpace($WebAppName)) {
        $WebAppName = "app-deep-research-$($ResourceGroup.ToLower())"
        Write-Info "Web App Name (auto-generated): $WebAppName"
    }
    
    $ImageName = "deep-research-app:latest"
    ...
    if (-not $SkipBuild) {
        ...
        try {
            docker build -t $ImageName -f deep_research_app/Dockerfile .
            if ($LASTEXITCODE -ne 0) {
