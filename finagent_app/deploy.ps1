# Unified Finagent App Deployment Script
...
param(
    ...
    [string]$McpContainerAppName = "yahoo-finance-mcp-fa",
    
    [Parameter(Mandatory=$false)]
    [string]$McpEnvironmentName = "finagent-mcp-env",
...
Write-Host "Finagent App - Unified Deployment" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$BackendImageName = "finagent-backend"
...
docker build -f finagent_app/Dockerfile -t $BackendImageName`:$BackendImageTag .
if ($LASTEXITCODE -ne 0) {
...
Write-Host "✓ Backend image pushed to ACR" -ForegroundColor Green
Write-Host ""
...
$BackendStaticPath = "$ScriptDir\backend\app\static"
...
Copy-Item -Recurse -Force "$ScriptDir\frontend\dist\*" $BackendStaticPath
Write-Host "✓ Frontend copied to backend" -ForegroundColor Green
Write-Host ""

# Rebuild backend image with frontend
Write-Host "Rebuilding backend image with frontend..." -ForegroundColor Yellow
Set-Location $parentDir
docker build -f finagent_app/Dockerfile -t $BackendImageName`:$BackendImageTag .
docker tag $BackendImageName`:$BackendImageTag $FullBackendImageName
docker push $FullBackendImageName
Write-Host "✓ Final image pushed to ACR" -ForegroundColor Green
Write-Host ""
