...
class MAFDynamicPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        logger.info("MAF planner initialised", agent=planner_agent.name)
    ...
    async def generate_plan(
        ...
        *,
        ...
        prompt = self._build_prompt(objective, files_info, summary_type, persona)
        logger.debug("Generating plan with MAF planner", objective_preview=objective[:80])
        ...
        logger.info("Plan generated via native MAF", steps=len(steps))
        return steps
    ...
    def _build_prompt(
        ...
        if files:
            lines = [f"- {f['filename']} ({f['file_type']})" for f in files]
            files_block = "\nUploaded Files:\n" + "\n".join(lines)
    ...
    def _default_rules() -> str:
        return (
            ...
            "1. Step 1 MUST process all uploaded files together using MultimodalProcessor_Agent.\n"
            "2. Include Sentiment_Agent only for audio/video or explicit sentiment requests.\n"
            "3. Summarizer_Agent and Analytics_Agent must include objective_context in parameters.\n"
            "4. Use format: Step N: <action>. Agent: <AgentName>. Tool: <tool>."
        )
    ...
    def _extract_text(response: AgentRunResponse) -> str:
        ...
        # Take the last message as the final answer
    ...
    def parse_plan_text(
        ...
        *,
        ...
        # When the LLM adds conversation preamble, keep only the numbered steps portion.
        ...
        for raw_line in step_lines:
            ...
            parameters = cls._enrich_parameters(parameters, agent, objective, summary_type, persona)
            if agent == "MultimodalProcessor_Agent":
                parameters.setdefault("file_ids", [f.get("file_id") for f in files_info])
            steps.append(
