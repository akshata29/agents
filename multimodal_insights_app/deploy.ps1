...
<#
.SYNOPSIS
    Deploy Multimodal Insights App to Azure App Service as Docker container
...
.PARAMETER WebAppName
    Azure Web App name (optional, defaults to app-multimodal-insights-{rg})
...
try {
    Write-Info "========================================="
    Write-Info "Multimodal Insights App - Azure Deployment"
    Write-Info "========================================="
    Write-Info ""
    ...
    if ([string]::IsNullOrWhiteSpace($WebAppName)) {
        $WebAppName = "app-multimodal-insights-$($ResourceGroup.ToLower())"
        Write-Info "Web App Name (auto-generated): $WebAppName"
    }
    
    $ImageName = "multimodal-insights-app:latest"
    ...
    if (-not $SkipBuild) {
        ...
        try {
            docker build -t $ImageName -f multimodal_insights_app/Dockerfile .
            if ($LASTEXITCODE -ne 0) {
